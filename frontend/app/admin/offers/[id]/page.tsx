...
export default function EditOfferPage() {
  ...
  const loadOffer = async () => {
    try {
      ...
      const offer = response.data.data.find((o: IOffer) => o._id === offerId);
      ...
      const startDate = new Date(offer.startDate).toISOString().slice(0, 16);
      const endDate = new Date(offer.endDate).toISOString().slice(0, 16);
      ...
      toastManager.error('Failed to load offer');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await api.updateOffer(offerId, offerData);
      console.log('Offer updated response:', response);

      toastManager.success('Offer updated successfully');
      router.push('/admin/offers');
    } catch (error: any) {
